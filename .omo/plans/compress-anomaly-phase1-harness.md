...
- [ ] 1. Scaffold uv project, layout, git init, docs skeleton
...
- [ ] 2. Metrics module: frozen threshold, point/event-F1, PR-AUC, VUS-PR
...
- [ ] 3. Datasets generator: 4 morphologies, seeded, frozen formulas
...
- [ ] 4. UCR series pin + loader with checksum, cache, freeze record
...
- [ ] 5. Codecs R0 + R3: lossless and scalar quantization
...
- [ ] 6. Codec R4: SAX/PAA symbolic with dual Path A/B
...
- [ ] 7. Detectors PCA + IsolationForest with frozen window alignment
...
- [ ] 8. Detector D4: direct-on-symbols Markov (R4 only)
...
- [ ] 9. Runner: full matrix, routing, CSV + Markdown table
...
- [ ] 10. Eval gates and hygiene audits
...
- [ ] 11. Docs finalize and docstring sweep
...
- [ ] 12. Full multi-seed matrix run, results commit
...
- [ ] F1. Plan compliance audit: every TASK_01 DoD item (TASK_01:267-274) + USER-LOCKED DELTAS (VUS-PR, pinned UCR, multi-seed, asserts) traced to a todo; fail on any untraced item
- [ ] F2. Code quality review: uv.lock committed (`test -f uv.lock` + pinned vus==0.0.6 present), no unpinned deps, no unseeded RNG (grep np.random\. + random\. without seed), docstrings on all public functions (sweep script green), PA-ban grep clean
- [ ] F3. Real manual QA: fresh `uv sync`, `--smoke` run, full-matrix spot check (Spike/IF Q8 vs R0 non-collapse + Rhythm grammar gate hold on means), CSV 14-col header validated, eval script exit codes verified on all five fixtures
- [ ] F4. Scope fidelity: no D2/D3/D5/FM/PATE/extra-rung/remote code present; Chaos+UCR excluded from asserts; per-dataset (never pooled) reporting confirmed
## Commit strategy
